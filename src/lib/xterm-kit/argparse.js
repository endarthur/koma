/**
 * xterm-kit: Argument Parser
 * Command-line argument parsing for terminal applications
 *
 * Extracted from koma's stdlib/args.js - battle-tested and feature-complete
 */
 ...
 * });
 */
export function parse(argv, schema = {}) {
  const result = {
    flags: {},
    options: {},
    positional: [],
 ...
 * });
 */
export function usage(commandName, schema = {}) {
  const lines = [];
 ...
 *
 * @example
 * if (showHelp('ls', args, schema, shell.term)) return;
 */
export function showHelp(commandName, argv, schema, term) {
  if (!hasHelp(argv)) {
    return false;
  }
 ...
 *
 * @example
 * hasFlag(args, '--verbose') // true if --verbose present
 * hasFlag(args, 'v') // true if -v present
 */
 ...
 *
 * @example
 * getOption(args, '--output', 'default.txt')
 * getOption(args, 'o', 'default.txt')
 */
export function getOption(argv, option, defaultValue = null) {
